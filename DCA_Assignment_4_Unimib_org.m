...
X = (1/78)*rand(78,3); Y = (1/78)*rand(78,3); alpha_b = 1/17*ones(17,1); v = 0.001; mu_2 = 100; K = 3;
...
for experiment_num = num_users
    ...
    if (range_overall > 10)
    clear data x_in gnd cxx M cxx_u
...
while k <= 300
    while 1
        ...
        while 1
            ...
            if norm(X_prev-X) < 0.6 && norm(alpha_bprev-alpha_b) < 0.6
                break;
            end
        end
        if norm(X_prev2-X_prev) < 0.6 && norm(alpha_bprev2-alpha_bprev) < 0.6
            break;
        end
    end
    ...
    result(k,:) = ClusteringMeasure(gnd,idx_X);
    %Matr = (X + Y)/2;
    %algo_acc = abs(Matr);
    
    %Method 2:
    %[val_Y,idx_Y] = max();
    %result2 = ClusteringMeasure(gnd,idx_Y)
    %M_1 = 0.5*(X*Y'+Y*X');
    %[U,D] = eig(M_1);
